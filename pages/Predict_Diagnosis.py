...
def medical_questionnaire():
    with st.form("medical_questionnaire"):
        ...
        if predict_10:
            ...
            st.write(f"**Predicted Diagonsis:** {convert_number_to_disease(prediction[0])} ({top3_diseases_prob[convert_number_to_disease(prediction[0])]*100:.2f}%)")
        if len(top3_diseases_prob) > 0:
            if max(list(top3_diseases_prob.values())) > 0.89:
                ...
                with st.form('More Information'):
                    ...
                    mobility = st.radio(
                        ...
                        ['Yes', 'No'], index=1)
                    inability_retain_protein = st.radio(
                        "Do you have a known kidney problem resulting in an inability to retain proteins?", ['Yes', 'No'],
                        index=1)
                    allergy_exposure = st.radio("Have you been in contact with or ate something that you have an allergy to?",
                                                ['Yes', 'No'], index=1)
                    ...
                    facial_muscles = st.radio("Have you noticed weakness in your facial muscles and/or eyes?", ['Yes', 'No'],
                                              index=1)
                    asthma_attacks = st.radio("Have you had 2 or more asthma attacks in the past year?", ['Yes', 'No'], index=1)
                    antipsychotic_medication = taken_antipsychotics
                    lung_cancer = st.radio("Do you have family members who have had lung cancer?", ['Yes', 'No'], index=1)
                    death_feeling = st.radio("Do you feel like you are dying or were you afraid that you were about to die?",
                                             ['Yes', 'No'], index=1)
                    bloated_swollen_abdomen = st.radio(
                        "Do you feel your abdomen is bloated or distended (swollen due to pressure from inside)?",
                        ['Yes', 'No'], index=1)
                    burning_sensation_in_stomach = st.radio(
                        ...
                        ['Yes', 'No'], index=1)
                    cluster_headaches = st.radio("Have any of your family members been diagnosed with cluster headaches?",
                                                 ['Yes', 'No'], index=1)
                    lesions_peel = st.radio("Do your lesions peel off?", ['Yes', 'No'], index=1)
                    sensation_in_feet = st.radio("Do you have numbness, loss of sensation, or tingling in the feet?",
                                                 ['Yes', 'No'], index=1)
                    intravenous_intramuscular_route = st.radio(
                        "Have you been treated in the hospital recently for nausea, agitation, intoxication or aggressive behavior and received medication via an intravenous or intramuscular route?",
                        ['Yes', 'No'], index=1)
                    gas_ability = st.radio("Have you been able to pass stools or gas since your symptoms increased?",
                                           ['Yes', 'No'], index=1)
                    irregular_heartbeat = st.radio(
                        "Do you feel your heart is beating very irregularly or in a disorganized pattern?", ['Yes', 'No'],
                        index=1)
                    face_paralysis = st.radio(
                        "Have you had weakness or paralysis on one side of the face, which may still be present or completely resolved?",
                        ['Yes', 'No'], index=1)
                    nose_throat_itchy = itchy_nose_throat
                    sight_problems = st.radio(
                        ...
                        ['Yes', 'No'], index=1)
                    menstrual_period = st.radio("Did you have your first menstrual period before the age of 12?", ['Yes', 'No'],
                                                index=1)
                    premature_birth = st.radio("Were you born prematurely or did you suffer any complication at birth?",
                                               ['Yes', 'No'], index=1)
                    ...
                    vomit = st.radio("Have you vomited several times or have you made several efforts to vomit?", ['Yes', 'No'],
                                     index=1)
                    sore_throat = st.radio("Do you have a sore throat?", ['Yes', 'No'], index=1)
                    sti_exposure = had_sti
                    symptoms_after_eating = st.radio("Do you have symptoms that get worse after eating?", ['Yes', 'No'],
                                                     index=1)
                    ...
                    muscle_spams = st.radio(
                        "Do you have annoying muscle spasms in your face, neck, or any other part of your body?", ['Yes', 'No'],
                        index=1)
                    OSA = st.radio("Have you ever been diagnosed with obstructive sleep apnea (OSA)?", ['Yes', 'No'], index=1)
                    similar_symptoms = st.radio(
                        "Have you been in contact with a person with similar symptoms in the past 2 weeks?", ['Yes', 'No'],
                        index=1)
                    ...
                    symptoms_progression = st.radio(
                        ...
                        ["Yes", "No"], index=1)
                    symptoms_increased = st.radio(
                        "Are the symptoms or pain increased with coughing, with an effort like lifting a weight or from forcing a bowel movement?",
                        ['Yes', 'No'], index=1)
                    lesions_redness = skin_issues
                    voice_tone = st.radio("Have you noticed that the tone of your voice has become deeper, softer or hoarse?",
                                          ['Yes', 'No'], index=1)
                    difficulty_speaking = st.radio("Do you have difficulty articulating words/speaking?", ['Yes', 'No'],
                                                   index=1)
                    ebola_exposure = st.radio(
                        "In the last month, have you been in contact with anyone infected with the Ebola virus?", ['Yes', 'No'],
                        index=1)
                    calcium_blockers_medication = st.radio("Do you take a calcium channel blockers (medication)?",
                                                           ['Yes', 'No'], index=1)
                    ...
                    pneumothorax_family = st.radio("Have any of your family members ever had a pneumothorax?", ['Yes', 'No'],
                                                   index=1)
                    ...
                    recent_asthma = st.radio("Have you been hospitalized for an asthma attack in the past year?", ['Yes', 'No'],
                                             index=1)
                    nose_polyps = st.radio("Do you have polyps in your nose?", ['Yes', 'No'], index=1)
                    crohns_disease = st.radio("Do you suffer from Crohn’s disease or ulcerative colitis (UC)?", ['Yes', 'No'],
                                              index=1)
                    pain_location = pain_related
                    ...
                    itchy_eyes = severe_eye_itching
                    anemia = diagnosis_of_anemia
                    ...
                    foreward_movement = st.radio("Do you have pain that improves when you lean forward?", ['Yes', 'No'],
                                                 index=1)
                    Rheumatoid_Arthritis = st.radio("Do you have Rheumatoid Arthritis?", ['Yes', 'No'], index=1)
                    diarrhea_stool_frequency = st.radio("Have you had diarrhea or an increase in stool frequency?",
                                                        ['Yes', 'No'], index=1)
                    croup = st.radio("Have you or any member of your family ever had croup?", ['Yes', 'No'], index=1)
                    saliva_production = st.radio("Have you noticed that you produce more saliva than usual?", ['Yes', 'No'],
                                                 index=1)
                    ...
                    pain_increase_movement = st.radio("Do you have pain that is increased with movement?", ['Yes', 'No'],
                                                      index=1)
                    ...
                    # Lifestyle and Environmental Factors
                    st.subheader("Other Lifestyle and Environmental Factors")

                    mood_stability = st.radio("Are you more irritable or has your mood been very unstable recently?",
                                              ['Yes', 'No'], index=1)
                    ...
                    cigarette_smoke_exposure = exposed_to_smoke
                    poor_diet = st.radio("Do you have a poor diet?", ['Yes', 'No'], index=1)
                    decongestants = st.radio(
                    ...
                    if predict_49:
                        ...
                        responses = process_responses_49(hiv_exposure, active_cancer, mood_stability, ulcers_sores,
                                                         ear_infection,
                                                         mobility, inability_retain_protein, allergy_exposure, chills_shivers,
                                                         food_allergy,
                                                         high_blood_pressure, red_cheeks, weight_loss, NOACs, unprotected_sex,
                                                         lymph_nodes,
                                                         facial_muscles, asthma_attacks, antipsychotic_medication, lung_cancer,
                                                         hormones_intake, death_feeling, bloated_swollen_abdomen,
                                                         burning_sensation_in_stomach,
                                                         cluster_headaches, lesions_peel, sensation_in_feet,
                                                         intravenous_intramuscular_route,
                                                         energy_drinks_consumption, gas_ability, irregular_heartbeat,
                                                         family_allergies,
                                                         face_paralysis, decongestants, nose_throat_itchy, sight_problems,
                                                         menstrual_period,
                                                         coffee_tea_consumption, premature_birth, hyperthyroidism,
                                                         cystic_fibrosis, vomit,
                                                         copd, poor_circulation, pneumothorax, itchy_eyes, symptoms_progression,
                                                         anemia, chronic_anxiety,
                                                         agriculture, blood_vessels, heart_defect, out_of_breath,
                                                         chronic_sinusitis,
                                                         foreward_movement, Rheumatoid_Arthritis, diarrhea_stool_frequency,
                                                         croup,
                                                         saliva_production, fluid_in_lungs, pericarditis, vomit_after_coughing,
                                                         pain_increase_movement, sore_throat, sti_exposure,
                                                         symptoms_after_eating,
                                                         chronic_kidney_failure, hiv, muscle_spams, OSA, mining_sector,
                                                         similar_symptoms, liver_cirrhosis, symptoms_at_night,
                                                         symptoms_increased,
                                                         lesions_redness, voice_tone, difficulty_speaking, ebola_exposure,
                                                         calcium_blockers_medication,
                                                         lesions_1cm, intravenous_drug_use, immunosuppressed,
                                                         pneumothorax_family,
                                                         excessive_tears, whooping_cough, tiredness, jaw_weakness, depression,
                                                         recent_asthma, nose_polyps, crohns_disease, construction,
                                                         pain_location,
                                                         chest_pain, dialysis, poor_diet, vaccinations,
                                                         cigarette_smoke_exposure,
                                                         decrease_appetite, rash_color, swelling_location)

                        # Generate prediction
                        prediction = model_49.predict(responses)

                        # Display the prediction
                        st.write(f"**Predicted Diagonsis:** {convert_48_diseases(prediction[0])}")

                        # Class probability
                        pred_class_prob = model_49.predict_proba(responses)
                        top_3_disease = np.argsort(-pred_class_prob, axis=1)[:, :3]
                        top3_diseases_prob = {}
                        # Extract the probabilities at these indices
                        for indices in top_3_disease:
